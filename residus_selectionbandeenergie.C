...
void MergeOffRuns(const char* filename,
		  ...
		  const char* outname, double Emin_band, double Emax_band,
		  double EnergieMin=0.,
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
void MergeOffRuns(const char* filename,TString path,const char* filename_RL,TString path_RL,TString Config,const char* outname, double Emin_band, double Emax_band, double EnergieMin,double EnergieMax)
{
  /*
    filename:run list
   ...
   EnergieMin, Energiemax:minimum et maximum des energies des evenements que l'n veut garder
   Emin_band, Emax_band: boudaries of the energy band selected to calculate the residus
  ...
  */
  ...
  std::vector<std::vector<TH2F*> >  hist_nomevents;
  std::vector<std::vector<TH2F*> >  hist_nomradiallookup;
  std::vector<std::vector<TH2F*> >  hist_residus;
  ...
  str_convert5 << Emin_band;
  str_convert6 << Emax_band;
  ...
  for (int izen=0;izen<Nbands;izen++) 
      ...
      for (int ieff=0;ieff<Nbands_eff;ieff++) 
	{
	  ...
	  hist_nomevents[izen][ieff] = new TH2F(tabname_events.c_str(),tabname_plot.c_str(),bin_nom, 0, nom_max, bin_nom, 0, nom_max);
	  hist_nomradiallookup[izen][ieff] = new TH2F(tabname_RL.c_str(),tabname_plot.c_str(),bin_nom, 0, nom_max, bin_nom, 0, nom_max);
	...
	if ( (fOffEvtEnergy >= EnergieMin) && (fOffEvtEnergy < EnergieMax) && (fOffEvtOffset < fEvtOffsetMax ) && (fOffEvtEnergy >= ethresh) ) 
	  {
	    ...
	    hist_nomevents[index][index_eff]->Fill(Xnom,Ynom);
  ...
  for (int izen=0;izen<Nbands;izen++) 
      ...
      for (int ieff=0;ieff<Nbands_eff;ieff++) 
	{
	  ...
	  for (int i_E=0;i_E<Nbands_E;i_E++) 
	    { 
	      if((E[i_E] > Emin_band) & (E[i_E] < Emax_band)){
		...
		for (int iX=1; iX <= bin_nom; iX++){
		  for (int iY=1; iY <= bin_nom; iY++){
		    Xaxis=hist_nomradiallookup[izen][ieff]->GetXaxis()->GetBinCenter(iX);
		    Yaxis=hist_nomradiallookup[izen][ieff]->GetYaxis()->GetBinCenter(iY);
		    ...
		    bin_number_2D=hist_nomradiallookup[izen][ieff]->FindBin(Xaxis,Yaxis);
		    hist_nomradiallookup[izen][ieff]->AddBinContent(bin_number_2D,events_RL);
		  }		  
		}				
	      }
	    }
	      //trouver koverwrite et comment changer nom suaver de histo pour pas remplacer
	  hist_nomevents[izen][ieff]->Scale(1/hist_nomevents[izen][ieff]->Integral());
	  hist_nomradiallookup[izen][ieff]->Scale(1/hist_nomradiallookup[izen][ieff]->Integral());
	  hist_nomevents[izen][ieff]->Write("",TObject::kOverwrite);
	  hist_nomradiallookup[izen][ieff]->Write("",TObject::kOverwrite);
	  c1=new TCanvas(name_canvas.c_str());
	  c1->Divide(2,2);
	  c1->cd(1);
	  hist_nomevents[izen][ieff]->Draw("colz");
	  c1->cd(2);
	  hist_nomradiallookup[izen][ieff]->Draw("colz");
	  
	  hist_nomevents[izen][ieff]->Add(hist_nomradiallookup[izen][ieff],-1);
	  hist_residus[izen][ieff]=(TH2F*)hist_nomevents[izen][ieff]->Clone();
	  hist_residus[izen][ieff]->SetName(name_hist_residus.c_str());
	  hist_residus[izen][ieff]->Write("",TObject::kOverwrite);
	  //surement fire un TPAD
	  
	  c1->cd(3);
	  hist_residus[izen][ieff]->Draw("colz");
